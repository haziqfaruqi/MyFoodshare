...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600 mt-1">Update your food listing details</p>
        </div>

        <div class="bg-white rounded-lg shadow-lg p-8">
            <form method="POST" action="{{ route('restaurant.listings.update', $listing) }}" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                ...
                                name="food_name"
                                required
                                value="{{ old('food_name', $listing->food_name) }}"
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="Prepared Meals" {{ old('category', $listing->category) === 'Prepared Meals' ? 'selected' : '' }}>Prepared Meals</option>
                                <option value="Bread & Bakery" {{ old('category', $listing->category) === 'Bread & Bakery' ? 'selected' : '' }}>Bread & Bakery</option>
                                <option value="Fruits & Vegetables" {{ old('category', $listing->category) === 'Fruits & Vegetables' ? 'selected' : '' }}>Fruits & Vegetables</option>
                                <option value="Dairy Products" {{ old('category', $listing->category) === 'Dairy Products' ? 'selected' : '' }}>Dairy Products</option>
                                <option value="Meat & Seafood" {{ old('category', $listing->category) === 'Meat & Seafood' ? 'selected' : '' }}>Meat & Seafood</option>
                                <option value="Beverages" {{ old('category', $listing->category) === 'Beverages' ? 'selected' : '' }}>Beverages</option>
                                <option value="Other" {{ old('category', $listing->category) === 'Other' ? 'selected' : '' }}>Other</option>
                            </select>
                            @error('category')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="pieces" {{ old('unit', $listing->unit) === 'pieces' ? 'selected' : '' }}>pieces</option>
                                <option value="kg" {{ old('unit', $listing->unit) === 'kg' ? 'selected' : '' }}>kg</option>
                                <option value="lbs" {{ old('unit', $listing->unit) === 'lbs' ? 'selected' : '' }}>lbs</option>
                                <option value="containers" {{ old('unit', $listing->unit) === 'containers' ? 'selected' : '' }}>containers</option>
                                <option value="boxes" {{ old('unit', $listing->unit) === 'boxes' ? 'selected' : '' }}>boxes</option>
                                <option value="bags" {{ old('unit', $listing->unit) === 'bags' ? 'selected' : '' }}>bags</option>
                                <option value="liters" {{ old('unit', $listing->unit) === 'liters' ? 'selected' : '' }}>liters</option>
                            </select>
                            @error('unit')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                ...
                                required
                                min="{{ date('Y-m-d') }}"
                                value="{{ old('expiry_date', $listing->expiry_date ? $listing->expiry_date->format('Y-m-d') : '') }}"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
                            />
                            @error('expiry_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                id="expiry_time"
                                name="expiry_time"
                                value="{{ old('expiry_time', $listing->expiry_time ? $listing->expiry_time->format('H:i') : '') }}"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
                            />
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                required
                                value="{{ old('pickup_location', $listing->pickup_location) }}"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
                        ...
                        <div>
                            ...
                            >{{ old('special_instructions', $listing->special_instructions) }}</textarea>
                        </div>
                    </div>
                </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    @foreach($listing->images as $image)
                                        <div class="relative">
                                            <img src="{{ asset('storage/' . $image) }}" alt="Food image" class="w-full h-24 object-cover rounded-lg">
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                ...
                <div class="flex items-center justify-end space-x-4">
                    <a
                        href="{{ route('restaurant.listings.index') }}"
